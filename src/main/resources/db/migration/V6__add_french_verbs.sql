...
ALTER TABLE leafs ADD CONSTRAINT leafs_leaf_type_check
  CHECK (leaf_type IN ('EsInf', 'EsNonverb', 'EsStemChange', 'EsUniqV', 'FrNonverb', 'FrInf', 'FrUniqV'));

ALTER TABLE leafs ADD CONSTRAINT leafs_fr_mixed_check
  CHECK ((leaf_type IN ('FrInf', 'FrUniqV', 'FrNonverb') AND fr_mixed IS NOT NULL) OR
    (leaf_type NOT IN ('FrInf', 'FrUniqV', 'FrNonverb') AND fr_mixed IS NULL));

ALTER TABLE leafs DROP CONSTRAINT leafs_en_past_check;
ALTER TABLE leafs ADD CONSTRAINT leafs_en_past_check
  CHECK ((leaf_type IN ('EsInf', 'EsStemChange', 'FrInf') AND en_past IS NOT NULL) OR
        (leaf_type NOT IN ('EsInf', 'EsStemChange', 'FrInf') AND en_past IS NULL));
...
ALTER TABLE leafs ADD CONSTRAINT leafs_infinitive_leaf_id_check
  CHECK ((leaf_type IN ('EsUniqV', 'EsStemChange', 'FrUniqV') AND infinitive_leaf_id IS NOT NULL) OR
        (leaf_type NOT IN ('EsUniqV', 'EsStemChange', 'FrUniqV') AND infinitive_leaf_id IS NULL));
...
ALTER TABLE leafs ADD CONSTRAINT leafs_tense_check
  CHECK ((leaf_type IN ('EsUniqV', 'EsStemChange') AND tense IN ('PRES', 'PRET')) OR
    (leaf_type IN ('FrUniqV') AND tense IN ('PRES')) OR
    (leaf_type NOT IN ('EsUniqV', 'EsStemChange', 'FrUniqV') AND person IS NULL));
